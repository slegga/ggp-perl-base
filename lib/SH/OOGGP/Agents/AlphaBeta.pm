...
Can only be used for 2 player match. Will perform bad else.
Uses the AlphaBeta algorithm.
...
Return a new fresh AlphaBeta agent.
...
Return info message. Name and status.
...
Clean agent up after unexpected end of match.
...
Do after work after a complete match
