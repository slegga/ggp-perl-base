package SH::GGP::Tools::StateMachine;
use Carp;
use Data::Dumper;
use Data::Compare;
use autodie;
...
#my @roles=();
# my %initstate=(); # initial state
# my %currstate=(); # continues variables with current values
#my %varstate =();  # current states variable
#my %action=();    # current action performed by role
#my @actionhistory=();
#my @rules=();     # Rules
...
sub get_init_state {
    ...
    if ( defined $world->{constants} ) {
        $return = dclone( $world->{constants} );
    }
    $return->{Constants} = $world->{constants};
    @$return{ keys %{ $world->{init} } } = values %{ $world->{init} };
    my $other = query_other( $world, $return, 'nil' );
    @$return{ keys %$other } = values %$other;
    my @legal = query_item( $world, $return, 'legal' );
    my $legal_hr;
    if (@legal) {
        $legal_hr = hashify(@legal);
    } elsif ( exists $return->{Constants}->{legal} ) {
        $legal_hr = $return->{Constants}->{legal};
    } else {
...
sub query_item {
    ...
    # return crossed list
    @return = get_result_fromrules( $world->{roles}, $state_hr, 'nil', @tmprules );
    if ( exists $state_hr->{Constants}->{$item} ) {
        my $const = dclone( $state_hr->{Constants}->{$item} );
...
sub query_other {
    ...
    for my $tmprule (@tmprules) {
        my @loop = get_result_fromarule( $world->{roles}, $return, $moves_ar, $tmprule );
...
sub query_nextstate {
    ...
    for my $rule (@tmprules) {
        if ( ( ref $rule->{effect} ne 'ARRAY' && $rule->{effect} eq $item ) || $rule->{effect}->[0] eq $item ) {
            last;
        }
    ...
    while ( my $rule = shift(@tmprules) ) {
        ...
        my @loop = get_result_fromarule( $world->{roles}, $state_hr, $moves, $rule );
    ...
    @return = get_result_fromrules( $world->{roles}, $state_hr, $moves, @tmprules );
...
sub get_result_fromarule {
    ...
    my $vars = SH::GGP::Tools::Variables->new();
    ...
    for my $tmpcriteria ( @{ $rule->{criteria} } ) {
        ...
        if ( $func eq 'true' ) {
            ...
            $vars->do_and( true( $state_hr, $criteria->[0], $vars ) );
        } elsif ( $func eq 'does' ) {
            $vars->do_and( does( $roles, $moves, $criteria ) );
        } elsif ( $func eq 'distinct' ) {
            $vars->do_and( $vars->distinct($criteria) );
        } elsif ( $func eq 'or' ) {
            $vars->do_or( _or_resolve( $state_hr, $criteria, $vars ), $rule->{effect} );
        } elsif ( $func eq 'not' ) {
            $vars->do_and( do_not( $state_hr, $criteria, $vars ) );
        } elsif (
            any {
                $func eq $_
            }
            ...
            )
        {
            ...
            $vars->do_and( true_varstate( $state_hr, $func, $criteria, $vars ) );
        }
    }
    my @return = get_effect( $rule->{effect}, $vars->get() );
...
sub get_result_fromrules {
    ...
    for my $rule (@tmprules) {
        ...
        for my $tmpcriteria ( @{ $rule->{criteria} } ) {
            next if !$vars->get_bool();
            if ( !ref $tmpcriteria ) {
                $vars->do_and( true_varstate( $state_hr, $tmpcriteria, undef, $vars ) );
            ...
            if ( $func eq 'true' ) {
                ...
                # recursive
                $vars->do_or( _or_resolve( $state_hr, $criteria, $vars ) );
            } elsif ( $func eq 'not' ) {
                $vars->do_and( do_not( $state_hr, $criteria, $vars, $rule->{effect} ) );
            } elsif (
                any {
                    $func eq $_
                }
        ...
        if ( $vars->get_bool() ) {
            push( @return, get_effect( $rule->{effect}, $vars->get() ) );
        }
...
sub _or_resolve {
    ...
    for my $func (@$inputs) {
        ...
        if ( ref $func eq 'ARRAY' ) {
            push( @$return, _or_resolve( $state_hr, $func, $vars ) );
        } elsif ( $func eq 'distinct' ) {
            shift @$inputs;
            $return = $vars->distinct($inputs);
            last;
        } elsif ( $func eq 'true' ) {
            ...
            )
        {
            ...
            shift @$inputs;
            $return = true_varstate( $state_hr, $func, $inputs, $vars );
        }
    }
    return $return;
}
...
sub true {
    ...
    return true_varstate( $state_hr, $statekey, $values_ar, $vars, $not );
}
...
sub true_varstate {
    ...
    if ( !defined $values_ar ) {
        ...
        ( $return, $filter ) = get_variable_n_filter($values_ar);
        ...
        if ( !exists $state_hr->{$statekey} ) {
            ...
            if ( !$not ) {    #true outside not
                ...
                for my $varsrow ( @{ $vars->{table} } ) {
                    ...
                    if ( !_true_if_row_exists( $state_hr, @testline ) ) {
                        push( @{ $return->{table} }, $newline );
                    }
                }
...
sub _rec_get_effect {
    ...
    for my $value (@$array_ref) {
        if ( ref $value eq 'ARRAY' ) {
            push( @$return_ar, _rec_get_effect( $value, $line, $variables ) );
        } elsif (
            any {
                $_ eq $value
            }
    ...
    if ( ref $effect eq 'ARRAY' ) {
        ...
        if ( !@evari ) {
            ...
            for my $line ( @{ $variables->{table} } ) {
                my $newline = _rec_get_effect( $effect, $line, $variables );
                push( @return, $newline );
            }
        }
        if ( @return > 65 ) {
...
Look for spesific moves.
Operate difrent than true.
...
sub does {
    ...
    my ( $return, $filter ) = get_variable_n_filter($values_ar);
    my @tmproles;
...
sub do_not {
    ...
    my $state    = 'true';
    my $table_ar = not_recresolve( $state_hr, $input, $vars );
...
sub not_recresolve {
    ...
    for my $func (@$inputs) {
        confess "\$func is not defined " . Dumper $inputs if !defined $func;
        if ( ref $func eq 'ARRAY' ) {
            push( @$return, not_recresolve( $state_hr, $func, $vars ) );
        } elsif ( $func eq 'distinct' ) {
            shift @$inputs;
            ...
            if ( ref $inputs->[1] eq 'ARRAY' ) {
                $return = true( $state_hr, $inputs->[1], $vars, 'not' );
            } else {
                shift @$inputs;
                $return = true( $state_hr, \@$inputs, $vars, 'not' );
            }
            last;
        } elsif (
            none {
            ...
            )
        {
            ...
            if ( none { substr( $_, 0, 1 ) eq '?' } @$inputs ) {
                $return = get_varstate_as_table( $state_hr, $func, \@tinput, 'not' );
            } else {    # (not (cellOpen ?x ?y1)))
                $return = true_varstate( $state_hr, $func, \@tinput, $vars, 'not' );
            }
            last;
        } else {
...
sub init_state_analyze {
    ...
    my @goals = query_item( $world, $state, 'goal' );
    if (@goals) {
...
sub process_move {
    ...
    for my $i ( 0 .. $#{$moves} ) {
        ...
        if ( none { Compare( $tmpmove, $_ ) } @tmplegal ) {
    ...
    my @next = query_nextstate( $world, $state_hr, $moves );
    ...
    my %contants = ();
    if ( exists $state_hr->{Constants} && defined $state_hr->{Constants} )
    {    #has to use camelcase for not do errors if a variable in rules is named contants
        %contants = %{ $state_hr->{Constants} };
    }
    $return_hr = dclone( \%contants );
    $return_hr->{Constants} = \%contants;
    my $new = hashify(@next);
    @$return_hr{ keys %$new } = values %$new;
    my $other = query_other( $world, $return_hr, $moves );
    @$return_hr{ keys %$other } = values %$other;

    # set_varstate($world,$state_hr);# mainpulates $state_hr

    my $is_terminal = query_item( $world, $return_hr, 'terminal' );
    if ($is_terminal) {
        $return_hr->{'terminal'} = '[true]';
        my @goals = query_item( $world, $return_hr, 'goal' );
        if ( !@goals ) {
        ...
        my $legal_hr;
        my @legal = query_item( $world, $return_hr, 'legal' );
        if (@legal) {
            $legal_hr = hashify(@legal);
        } elsif ( exists $return_hr->{Constants}->{legal} ) {
            $legal_hr = $return_hr->{Constants}->{legal};
        } else {
            confess "Cant find legal moves ";
        }
        $return_hr->{'legal'} = $legal_hr;
        if ( any { !exists $return_hr->{'legal'}->{$_} } @{ $state_hr->{role} } ) {
            logf( data_to_gdl($state_hr) );
