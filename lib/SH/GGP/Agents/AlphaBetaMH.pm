package SH::GGP::Agents::AlphaBetaMH;
use strict;
use warnings;
use autodie;
use Data::Dumper;
...
#    warn "ABM 82\n". Dumper $world->{analyze};
    $self->{'searchlevel'} = ($world->{analyze}->{firstmoves}>1 ? int(log($mconst)/log($world->{analyze}->{firstmoves})) : log($mconst));
    $self->loginfo('START');
    $self->init($world,$sc,$pc);
...
sub play {
    ...
    $self->loginfo("TURN:".$self->{'turn'}." Actions: ".scalar(@actions). " searchlevel: ".$self->{'searchlevel'});
    if (!$#actions) {
...
Loop trhou all move warings with a while
...
my ($self,$level,$state) = @_;
    ...
    #for my $i (0 .. $#actions) {
    while (defined $i) {
        my $result = $self->minscore($self->{'role'},$level,$actions[$i],$state,-1,101);
...
sub maxscore {
   ...
   confess("missing input got ".scalar(@_) ." ecpected 6") if @_<6;
    ...
    for my $i(0 .. $#actions) {
      my $result = $self->minscore($role,$level,$actions[$i],$state,$alpha,$beta);
...
#         }
       my $result = $self->maxscore($role,$level,$newstate,$alpha,$beta);
