package SH::GGP::Agents::MCTSLight;
use strict;
use warnings;
use autodie;
use Data::Dumper;
...
#    my @retactions = @curractions;
    ...
    for my $i(0 .. $#roles) {
        if (!defined $allactions[$i][$moveno->[$i]]{value} || $scores->[$i] < $allactions[$i][$moveno->[$i]]{value} ) {
    ...
    for my $i(0 .. $#roles) {
        for my $j(0 ..$#{$allactions[$i]}) {
            ...
            } elsif (defined $allactions[$i][$j]{value} && defined $moves[$i]{value} && $moves[$i]{value} < $allactions[$i][$j]{value}) {
                $moves[$i] = $allactions[$i][$j];
            }
        }
    }
